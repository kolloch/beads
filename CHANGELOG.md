...
- **`bd daemon` — long-lived Unix-socket query daemon (Layer 2).** `bd daemon start` runs a process that holds a single read-only Dolt connection open and serves the hot read-only commands (`ready`, `show`, `stats`, `blocked`) over a Unix-domain socket, so callers stop paying a fresh auth handshake + server-side per-connection thread setup on every poll. Steady-state Dolt connection churn under ambient agent polling collapses from dozens/sec to ~1. Subcommands: `start [--foreground] [--idle-timeout]`, `status`, `ping`, `stop`. The socket speaks a length-prefixed JSON protocol documented in [`docs/DAEMON.md`](docs/DAEMON.md) (the contract the `gc` sidecar consumes). Read-only scope only; generic-subcommand execution and write-commit batching are tracked as follow-ups (running arbitrary `cmd/bd` logic in-process is blocked by the ~233 `os.Exit` sites). Builds on the Layer 1 `READ_ONLY` classification. (be-3t4; Layer 1 be-y9p)
...
- **Read-only subcommands no longer issue tip-metadata `DOLT_COMMIT` calls.** `bd ready / list / show / search / stats / blocked / count / graph / duplicates / comments / current / ping / backup / export / context` are now classified as `READ_ONLY` at the dispatcher (single source of truth in `cmd/bd/intent.go`); their pending `tip_*_last_shown` writes accumulate in the working set and ride along on the next mutating command's commit. Eliminates the spurious `nothing to commit` entries from these paths. Layer 1 of the dispatcher-level read/write classification work tracked in `be-y9p`.
